...
interface DeleteProductModalProps {
    ...
    onClose: () => void;
    onConfirm: () => void;
    productName: string | undefined; // Allow undefined initially
    isLoading: boolean;
}

export default function DeleteProductModal({ isOpen, onClose, onConfirm, productName, isLoading }: DeleteProductModalProps) {
    ...
    return (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-opacity-50 backdrop-blur-sm" onClick={handleBackgroundClick}>
            <div className="bg-white dark:bg-zinc-800 p-6 rounded-3xl shadow-xl w-full max-w-md border border-zinc-700">
                ...
                <div className="mb-6">
                    <p className="text-sm text-gray-700 dark:text-gray-300">Are you sure you want to delete the product "{productName || 'this product'}"? This action cannot be undone.</p>
                </div>
